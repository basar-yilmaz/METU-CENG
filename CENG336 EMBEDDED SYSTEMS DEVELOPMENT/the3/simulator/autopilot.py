...
class AutoPilot:
    ...
    def reader_worker(self):
        ...
        while self.alive:
            ...
            if len(byte) == 0:
                print(f"Reader has timed out. Timeout was {self.serial.timeout}")
                continue
    ...
    def setup_periodicity_agent(self, testcase):
        ...
        for controller_idx, controls in enumerate(testcase["altitude-controls"]):
            periodicity_agent.add_periodic_agent(
                AltitudeControllerAgent(controller_idx, testcase, self)
            )
    ...
    def agents_demo(self):
        ...
        periodicity_agent: PeriodicityAgent = self.setup_periodicity_agent(TESTCASE)
...
def main():
    ap = AutoPilot(PORT, BAUDRATE, "N", rtscts=False, xonxoff=False)
